...
- 🔄 **WebSocket长连接**: 从机通过单一WebSocket连接向主机报告状态和数据（消除TCP短连接问题）
...
**HTTP Server (Express)**
- API 路由：`/api/*`（任务管理、场景管理、证书管理等）
- 静态文件：前端构建产物（`frontend/dist/`）
- 健康检查：`/health`（返回系统状态）
...
**HTTP Server (Express)**
- API 路由：`/api/*`（仅供主机调用：启动任务、停止任务、获取统计等）
- 健康检查：`/health`（主机监控从机存活）
- **无静态前端服务**（从机无 Web UI）

**WebSocket Client (Socket.IO)**
- **连接到主机:3000（单一长连接）**
- 发送 `slave:register`：首次连接或重连时注册自己（ID、名称、IP、端口）
- 发送 `slave:heartbeat`：低频周期（60秒）+ 按需上报（任务变化时）
- 发送 `slave:offline`：主动断开前通知主机
- 监听 `status:request`：响应主机的状态查询请求
- **无 WebSocket Server**（从机不接受连接）
- **连接保活**：依赖 Socket.IO 内置 ping/pong（25秒间隔），无冗余应用层心跳
...
4. **按需上报**：状态上报采用低频周期（60秒）+ 事件驱动（任务变化时立即上报）双重机制
...
- 状态上报采用低频周期（60秒）+ 事件驱动（任务变化时立即上报）双重机制
- 60秒周期心跳保证 `last_heartbeat` 字段持续更新，便于前端监控
- 连接失败次数会被记录在日志中供调试使用
...
**主机任务历史：** 显示所有节点的任务（包括主机和所有从机）
- 这是**预期行为**，方便集中管理和查看所有任务
- 通过 "执行机器" 列可以区分任务来源
- 可以使用表格筛选功能只查看特定机器的任务
...
- ✨ 新增 TLS/DTLS 证书管理功能
- 🔧 RTP 端口范围改为可选配置
- 🔄 **心跳机制优化为低频周期 + 按需双重机制**（消除 TCP 短连接导致的 TIME_WAIT 堆积）
- 🚀 从机通过单一 WebSocket 连接完成所有通信（注册、状态上报）
- ⚡ **状态上报采用60秒低频周期 + 任务变化时立即上报**（保证监控实时性与数据库字段更新）
- 📊 **保留低频心跳确保 last_heartbeat 字段更新**（便于前端监控，减少网络开销）
